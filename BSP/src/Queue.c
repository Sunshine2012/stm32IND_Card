...
/**********************************************************************
  * 函数名称:   u8 canInitQueue (CanQueue * p_tQueue)
  * 功能描述:   初始化队列
  * 输入参数:   CanQueue * tRxQueue: 队列指针
  * 输出参数:
  * 返 回 值:   成功为0
  * 创 建 者:   彭杰
  * 创建时间:   20140512
...
u8 canInitQueue( CanQueue * p_tQueue )
{
    ...
    p_tQueue->empty = 1;                // 队列为空
    p_tQueue->full = 0;                 // 队列为满
    p_tQueue->top = 0;                  // 队列的下标指针
    p_tQueue->bottom = 0;               // 队列的目标指针
    return 0;
}

/**********************************************************************
  * 函数名称:   u8 canInQueue (CanQueue * const p_tQueue, CanRxMsg * const p_tNewNode)
  * 功能描述:   数据入队列,这里设置的为循环队列,队列大小为RX_DATA_BUFF_SIZE,当数据存满了,将队头的数据删除
  * 输入参数:   CanQueue * tRxQueue: 队列指针, CanRxMsg * const p_tNewNode:即将入队列的数据指针
  * 输出参数:
...
/**********************************************************************
  * 函数名称:   u8 canOutQueue (CanQueue * const p_tQueue, CanRxMsg * p_tReturnNode)
  * 功能描述:   出队列
  * 输入参数:   CanQueue * const p_tQueue:队列首地址, CanRxMsg * p_tReturnNode:出队列元素保存地址
  * 输出参数:   出队列元素
  * 返 回 值:   成功为0
  * 创 建 者:   彭杰
  * 创建时间:   20140512
...
/**********************************************************************
  * 函数名称:   u8 uartInitQueue (UartQueue * tQueue)
  * 功能描述:   初始化队列
  * 输入参数:   CanQueue * tRxQueue: 队列指针
  * 输出参数:
  * 返 回 值:   成功为0
